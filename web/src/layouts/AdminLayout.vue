...
import {
  ...
  NButton, NAvatar, NDropdown, NSpace, NText, NIcon, NTag, NModal,
  NForm, NFormItem, NInput, useMessage
} from 'naive-ui'
import {
  DesktopOutline, ServerOutline, PeopleOutline, LogOutOutline,
  ImageOutline, SettingsOutline, MegaphoneOutline, HomeOutline, KeyOutline,
  TicketOutline, GlobeOutline
...
const menuOptions: MenuOption[] = [
  { label: '仪表盘', key: 'AdminDashboard', icon: renderIcon(DesktopOutline) },
  { label: 'VM 管理', key: 'AdminVMList', icon: renderIcon(ServerOutline) },
  { label: '用户管理', key: 'AdminUsers', icon: renderIcon(PeopleOutline) },
  { label: '镜像管理', key: 'AdminImages', icon: renderIcon(ImageOutline) },
  { label: '网络配置', key: 'AdminNetwork', icon: renderIcon(GlobeOutline) },
  { label: '公告管理', key: 'AdminAnnouncements', icon: renderIcon(MegaphoneOutline) },
  { label: '邀请码', key: 'AdminInviteCodes', icon: renderIcon(TicketOutline) },
  { label: '网站配置', key: 'AdminSettings', icon: renderIcon(SettingsOutline) },
  { type: 'divider' },
  ...
  <n-layout has-sider style="height: 100vh">
    ...
    <n-layout>
      <n-layout-header bordered style="height: 56px; display: flex; align-items: center; justify-content: space-between; padding: 0 24px">
        <n-tag type="warning" size="small">管理员</n-tag>
        <n-space align="center">
          <n-text>{{ userStore.userInfo?.username }}</n-text>
          <n-dropdown :options="userDropdownOptions" @select="handleUserDropdown">
            <n-button quaternary circle>
              <n-avatar :size="32" round>{{ userStore.userInfo?.username?.[0]?.toUpperCase() || 'A' }}</n-avatar>
            </n-button>
          </n-dropdown>
        </n-space>
      </n-layout-header>
